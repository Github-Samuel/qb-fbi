...
local StatusList = {
    ...
    ['redeyes'] = 'Red eyes',
    ['weedsmell'] = 'Smells like grass',
    ['gunpowder'] = 'Gunpowder on clothing',
    ['chemicals'] = 'Smells like chemicals',
    ['heavybreath'] = 'Breathe Strong',
    ['sweat'] = 'Sweats a lot',
    ['handbleed'] = 'Blood on the hands',
    ['confused'] = 'Confused',
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    local blooddropList = {}
    QBCore.Functions.Progressbar('clear_blooddrops', 'Cleaning the blood..', 5000, false, true, {
        ...
        if Blooddrops and next(Blooddrops) then
            ...
            QBCore.Functions.Notify('Clean blood :)')
        end
    end, function() -- Cancel
        QBCore.Functions.Notify('Blood not cleaned..', 'error')
    end)
end)
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    QBCore.Functions.Progressbar('clear_casings', 'Removing bullet casings..', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings and next(Casings) then
            ...
            QBCore.Functions.Notify('Bullet casings removed :)')
        end
    end, function() -- Cancel
        QBCore.Functions.Notify('Bullet casings not removed', 'error')
    end)
end)
