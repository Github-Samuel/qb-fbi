...
RegisterNetEvent('fbi:client:spawnCone', function()
    QBCore.Functions.Progressbar("spawn_object", "Colocar el objeto..", 2500, false, true, {
...
RegisterNetEvent('fbi:client:spawnBarier', function()
    QBCore.Functions.Progressbar("spawn_object", "Colocar el objeto..", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("fbi:server:spawnObject", "barier")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("Canceled..", "error")
    end)
...
RegisterNetEvent('fbi:client:spawnSchotten', function()
    QBCore.Functions.Progressbar("spawn_object", "Colocar el objeto..", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("fbi:server:spawnObject", "schotten")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("Canceled..", "error")
    end)
...
RegisterNetEvent('fbi:client:spawnLight', function()
    QBCore.Functions.Progressbar("spawn_object", "Place the object..", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('fbi:client:deleteObject', function()
    ...
    if dist < 5.0 then
        QBCore.Functions.Progressbar("remove_object", "Removal of the object..", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
RegisterNetEvent('fbi:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        ...
        QBCore.Functions.Notify('No spikes left..', 'error')
    end
end)
            ...
            if ClosestSpike then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.name == "fbi" and PlayerJob.onduty then
                            sleep = 0
                            DrawText3D(pos.x, pos.y, pos.z, '[E] Remove Skewer')
